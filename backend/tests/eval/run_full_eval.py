#!/usr/bin/env python3
"""Full evaluation runner — tests all fixtures and produces a detailed report.

Usage:
    cd backend && PYTHONPATH=src:. python tests/eval/run_full_eval.py
"""
...
def run_single(name: str) -> dict:
    ...
    t0 = time.time()
    entities, rels = extract_entities_nlp(text, doc_id=f"eval-{name}")
...
def main():
    ...
    print(f"Discovered {len(fixtures)} test documents: {fixtures}")
